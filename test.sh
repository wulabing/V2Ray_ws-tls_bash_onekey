...
ssl_judge_and_install() {
    ...
    echo -e " 2. 将重命名后的证书文件放入/data/目录后再运行脚本"
    echo -e " 3. 重新运行脚本"
    ...
    *)
        ...
        if [[ -f "/data/v2ray.key" && -f "/data/v2ray.crt" ]] && [[ -f "$HOME/.acme.sh/${domain}_ecc/${domain}.key" && -f "$HOME/.acme.sh/${domain}_ecc/${domain}.cer" ]]; then
            echo -e "${GreenBG} 所有证书文件均已存在, 是否保留 [${Red}Y${Font}${GreenBG}/N]? ${Font}"
            read -r ssl_delete_1
            case $ssl_delete_1 in
            [nN][oO] | [nN])
                delete_tls_key_and_crt
                rm -rf /data/v2ray.key
				rm -rf /data/v2ray.crt
                echo -e "${OK} ${GreenBG} 已删除 ${Font}"
                ssl_install
                acme
                ;;
            *)
                chown -fR nobody:${cert_group} /data/v2ray.*
                judge "证书应用"
                ;;
            esac
        elif [[ -f "/data/v2ray.key" || -f "/data/v2ray.crt" ]] && [[ ! -f "$HOME/.acme.sh/${domain}_ecc/${domain}.key" && ! -f "$HOME/.acme.sh/${domain}_ecc/${domain}.cer" ]]; then
        ...
        elif [[ -f "$HOME/.acme.sh/${domain}_ecc/${domain}.key" && -f "$HOME/.acme.sh/${domain}_ecc/${domain}.cer" ]] && [[ ! -f "/data/v2ray.key" || ! -f "/data/v2ray.crt" ]]; then
